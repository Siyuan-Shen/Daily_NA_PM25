...
class CNN_Transformer_InputDatasets():
    def __init__(self, species, cnn_channel_names, transformer_channel_names, bias, normalize_bias, normalize_species, absolute_species, datapoints_threshold):
        self.datapoints_threshold = datapoints_threshold

        self.species = species
        self.cnn_channel_names = cnn_channel_names
        self.transformer_channel_names = transformer_channel_names
        ...
        print('Start loading and aggregating the CNN training datasets and Transformer training datasets...')
        self.CNN_trainingdatasets = self._load_aggregate_TrainingDatasets(self.cnn_channel_names, CNN_Transformer_Training_infiles, self.total_sites_number)
        self.Transformer_trainingdatasets = self._load_aggregate_TrainingDatasets(self.transformer_channel_names, Transformer_Training_infiles, self.total_sites_number)
        print('The time of loading and aggregating the training datasets is {} seconds'.format(time.time()-start_time))
        self.ground_observation_data, self.geophysical_species_data, self.bias_data, self.CNN_trainingdatasets, self.Transformer_trainingdatasets, self.true_input = self._rename_key_in_dict()
        self.CNN_trainingdatasets_mean, self.CNN_trainingdatasets_std = self._derive_trainingdatasets_normalization_matrix(self.CNN_trainingdatasets)
        self.Transformer_trainingdatasets_mean, self.Transformer_trainingdatasets_std = self._derive_trainingdatasets_normalization_matrix(self.Transformer_trainingdatasets)

        self.width, self.height = self.CNN_trainingdatasets_mean.shape[1], self.CNN_trainingdatasets_mean.shape[2]
    ...
    def _process_concatenate_site_data(self, data, temp_data, isite):
        ...
        return data
    def _load_aggregate_TrainingDatasets(self, Training_Channels, Training_files, sites_number):
        '''
        ...
        for i, channel_name in enumerate(Training_Channels):
            infile = Training_files.format(channel_name)
            if os.path.exists(infile):
                temp_data = np.load(infile, allow_pickle=True).item()
            else:
    ...
    def _derive_trainingdatasets_normalization_matrix(self,datasets):
       
        # Collect all site data in a list
        datasets = [datasets[str(isite)]['data'] for isite in self.ground_observation_data.keys()]
    ...
    def _rename_key_in_dict(self):
        ...
        temp_bias_data = {}
        temp_CNN_trainingdatasets = {}
        temp_Transformer_trainingdatasets = {}
        temp_true_input = {}
        for isite, site in enumerate(self.ground_observation_data.keys()):
            ...
            temp_CNN_trainingdatasets[str(isite)] = self.CNN_trainingdatasets[site]
            temp_Transformer_trainingdatasets[str(isite)] = self.Transformer_trainingdatasets[site]
            temp_true_input[str(isite)] = self.true_input[site]
        return temp_observation_data, temp_geophysical_species_data, temp_bias_data, temp_CNN_trainingdatasets, temp_Transformer_trainingdatasets, temp_true_input
    ...
    def get_desired_range_inputdatasets(self,start_date,end_date,max_len=365,spinup_len=30):
        ...
        desired_CNN_trainingdatasets = copy.deepcopy(self.CNN_trainingdatasets)
        desired_Transformer_trainingdatasets = copy.deepcopy(self.Transformer_trainingdatasets)
        ...
        for isite in self.ground_observation_data.keys():
            ...
            temp_desired_CNN_trainingdatasets = np.full((number_of_batch_each_site, max_len+ spinup_len, self.CNN_trainingdatasets[str(isite)]['data'].shape[1], self.width,self.height), 0.0, dtype=np.float32)
            temp_desired_Transformer_trainingdatasets = np.full((number_of_batch_each_site, max_len+ spinup_len, self.Transformer_trainingdatasets[str(isite)]['data'].shape[1]), 0.0, dtype=np.float32)
            ...
            for i in range(number_of_batch_each_site):
                ...
                if len(np.where(desired_true_input[site]['dates'] >= temp_start_date)[0]) == 0 or len(np.where(desired_true_input[site]['dates'] <= temp_end_date)[0]) == 0:
                    ...
                    trainingdatasets_start_index = np.where(desired_CNN_trainingdatasets[site]['dates'] >= temp_start_date_with_spinup)[0][0]
                    trainingdatasets_end_index = np.where(desired_CNN_trainingdatasets[site]['dates'] <= temp_end_date)[0][-1]
                    ...
                    training_batch_indices = np.where(np.isin(batch_dates,desired_CNN_trainingdatasets[site]['dates'][trainingdatasets_start_index:trainingdatasets_end_index+1]))[0]
                    ...
                    temp_desired_CNN_trainingdatasets[i,training_batch_indices,:] = self.CNN_trainingdatasets[site]['data'][trainingdatasets_start_index:trainingdatasets_end_index+1,:]
                    temp_desired_Transformer_trainingdatasets[i,training_batch_indices,:] = self.Transformer_trainingdatasets[site]['data'][trainingdatasets_start_index:trainingdatasets_end_index+1,:]
            ...
            desired_CNN_trainingdatasets[site]['data'] = temp_desired_CNN_trainingdatasets
            desired_Transformer_trainingdatasets[site]['data'] = temp_desired_Transformer_trainingdatasets
            desired_geophysical_species_data[site]['geoPM25'] = temp_desired_geophysical_species_data

            desired_CNN_trainingdatasets[site]['dates'] = total_dates_array        
            desired_Transformer_trainingdatasets[site]['dates'] = total_dates_array
        ...
        return desired_CNN_trainingdatasets, desired_Transformer_trainingdatasets, desired_true_input,  desired_ground_observation_data, desired_geophysical_species_data
    
    def normalize_trainingdatasets(self,desired_CNN_trainingdatasets, desired_Transformer_trainingdatasets):
        ...
        desired_normalized_CNN_trainingdatasets = copy.deepcopy(desired_CNN_trainingdatasets)
        desired_normalized_Transformer_trainingdatasets = copy.deepcopy(desired_Transformer_trainingdatasets)
        for isite in self.ground_observation_data.keys():
            desired_normalized_CNN_trainingdatasets[str(isite)]['data'] = (desired_normalized_CNN_trainingdatasets[str(isite)]['data'] - self.CNN_trainingdatasets_mean) / self.CNN_trainingdatasets_std
            desired_normalized_Transformer_trainingdatasets[str(isite)]['data'] = (desired_normalized_Transformer_trainingdatasets[str(isite)]['data'] - self.Transformer_trainingdatasets_mean) / self.Transformer_trainingdatasets_std
        return desired_normalized_CNN_trainingdatasets, desired_normalized_Transformer_trainingdatasets

    def concatenate_trainingdatasets(self,desired_true_input, desired_normalized_CNN_trainingdatasets,desired_normalized_Transformer_trainingdatasets,desired_ground_observation_data, desired_geophysical_species_data):
        ...
        datasets = [desired_normalized_CNN_trainingdatasets[str(isite)]['data'] for isite in self.ground_observation_data.keys()]
        total_CNN_trainingdatasets = np.concatenate(datasets, axis=0).astype(np.float32)

        datasets = [desired_normalized_Transformer_trainingdatasets[str(isite)]['data'] for isite in self.ground_observation_data.keys()]
        total_Transformer_trainingdatasets = np.concatenate(datasets, axis=0).astype(np.float32)
        ...
        datasets = [desired_normalized_CNN_trainingdatasets[str(isite)]['dates'] for isite in self.ground_observation_data.keys()]
        total_dates = np.concatenate(datasets, axis=0).astype(np.float32)

        datasets = [int(isite)*np.ones(len(desired_normalized_CNN_trainingdatasets[str(isite)]['dates']),dtype=int) for isite in self.ground_observation_data.keys()]
        ...
        total_CNN_trainingdatasets = total_CNN_trainingdatasets[~nan_indices]
        total_Transformer_trainingdatasets = total_Transformer_trainingdatasets[~nan_indices]
        ...
        return total_CNN_trainingdatasets, total_Transformer_trainingdatasets, total_true_input, total_ground_observation_data,total_geophysical_species_data, total_sites_index, total_dates
