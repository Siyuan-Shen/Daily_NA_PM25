...
def Hyperparameters_Search_Training_Testing_Validation(wandb_config,total_channel_names,main_stream_channel_names,side_stream_channel_names,):
    ...
    if not Use_recorded_data_to_show_validation_results:
            ...
            Training_losses_recording, Training_acc_recording, valid_losses_recording, valid_acc_recording = initialize_Loss_Accuracy_Recordings(kfolds=1,n_models=len(HSV_Spatial_splitting_begindates),epoch=epoch,batchsize=batchsize)
                ...
                for imodel in range(len(HSV_Spatial_splitting_begindates)):
                    ...
                    if Apply_CNN_architecture:
                        ...
                        save_daily_datesbased_model(model=Daily_Model,evaluation_type=Evaluation_type,typeName=typeName,
                                                begindates=HSV_Spatial_splitting_begindates[imodel],enddates=HSV_Spatial_splitting_enddates[imodel],
                                                version=version,species=species,nchannel=len(main_stream_channel_names),width=width,height=height,
                                                special_name=description,ifold=0)
                    ...
                    if Apply_3D_CNN_architecture:
                        ...
                        save_daily_datesbased_model(model=Daily_Model,evaluation_type=Evaluation_type,typeName=typeName,
                                                ...
                                                special_name=description,ifold=0,width=width,height=height,depth=depth)
