...
from Evaluation_pkg.data_func import Split_Datesets_based_on_dates,Split_Datasets_based_site_index,randomly_select_training_testing_indices,Get_final_output
...
def temporal_cross_validation(total_channel_names, main_stream_channel_names,
    ...
    Evaluation_type = 'Temporal_CrossValidation'
    ...
    seed       = 19980130
    rkf = RepeatedKFold(n_splits=Temporal_CV_folds, n_repeats=1, random_state=seed)
    manager = Manager()
    ...
    if not Use_recorded_data_to_show_validation_results_Temporal_CV:
            ...
            for imodel in range(len(Temporal_CV_training_begindates)):
                    ...
                    if Apply_CNN_architecture or Apply_3D_CNN_architecture:
                        ...
                        print('1...',' Start Date: ', Temporal_CV_training_begindates[imodel], ' End Date: ', Temporal_CV_training_enddates[imodel])
                        desired_trainingdatasets, desired_true_input,  desired_ground_observation_data, desired_geophysical_species_data = Init_CNN_Datasets.get_desired_range_inputdatasets(start_date=Temporal_CV_training_begindates[imodel],
                                                                                                        end_date=Temporal_CV_training_enddates[imodel]) # initial datasets
                        ...
                        # Get the initial true_input and training datasets for the current model (within the desired time range)
                        print('1...',' Start Date: ', Temporal_CV_training_begindates[imodel], ' End Date: ', Temporal_CV_training_enddates[imodel])
                        desired_trainingdatasets, desired_true_input,  desired_ground_observation_data, desired_geophysical_species_data = Init_Transformer_Datasets.get_desired_range_inputdatasets(start_date=Temporal_CV_training_begindates[imodel],
                                                                                                        end_date=Temporal_CV_training_enddates[imodel],max_len=max_len,spinup_len=spin_up_len)
                        # Normalize the training datasets
                        print('2...', 'Desired Training Datasets: ', desired_trainingdatasets.keys())
                        ...
                        # Get the initial true_input and training datasets for the current model (within the desired time range)
                        print('1...')
                        desired_CNN_trainingdatasets, desired_Transformer_trainingdatasets, desired_true_input,  desired_ground_observation_data, desired_geophysical_species_data = Init_CNN_Datasets.get_desired_range_inputdatasets(start_date=Temporal_CV_training_begindates[imodel],
                                                                                                        end_date=Temporal_CV_training_enddates[imodel],max_len=max_len,spinup_len=spin_up_len)
                        # Normalize the training datasets
                        print('2...')
                    ...
                    dates_range, int_dates_range = create_date_range(Temporal_CV_training_begindates[imodel], Temporal_CV_training_enddates[imodel]) 
                    ### Start folds loop
                    for ifold, (training_dates_index,testing_dates_index) in enumerate(rkf.split(int_dates_range)):
                        training_dates = int_dates_range[training_dates_index]
                        testing_dates = int_dates_range[testing_dates_index]
                        print('training_dates_index: ', training_dates_index.shape)
                        print('testing_dates_index: ', testing_dates_index.shape) 
                        ...
                        if Apply_3D_CNN_architecture or Apply_CNN_architecture or Apply_Transformer_architecture:
                            X_train, y_train, X_test, y_test, dates_train, dates_test, sites_train, sites_test, train_datasets_index, test_datasets_index = Split_Datesets_based_on_dates(train_dates=training_dates,
                                                                                                                                            test_dates=testing_dates,
                            ...
                            X_train_CNN, y_train, X_test_CNN, y_test, dates_train, dates_test, sites_train, sites_test, train_datasets_index, test_datasets_index = Split_Datesets_based_on_dates(train_dates=training_dates,
                                                                                                                                        test_dates=testing_dates,
                                                                                                                                        total_trainingdatasets=cctnd_CNN_trainingdatasets,
                                                                                                                                        total_true_input=cctnd_true_input,
                                                                                                                                        total_sites_index=cctnd_sites_index,
                                                                                                                                        total_dates=cctnd_dates)
                            X_train_Transformer, y_train, X_test_Transformer, y_test, dates_train, dates_test, sites_train, sites_test, train_datasets_index, test_datasets_index = Split_Datesets_based_on_dates(train_dates=training_dates,
                                                                                                                                        test_dates=testing_dates,
                        ...
                        ## 2D CNN Training
                        if Apply_CNN_architecture:
                            if not Use_saved_models_to_reproduce_validation_results_Temporal_CV:
                                if world_size > 1:
                                    mp.spawn(CNN_train,args=(world_size,temp_sweep_config,sweep_mode,sweep_id,run_id_container,total_channel_names,X_train, y_train,\
                                                    ...
                                                    Evaluation_type,typeName,Temporal_CV_training_begindates[imodel],\
                                                    Temporal_CV_training_enddates[imodel],ifold),nprocs=world_size)
                                else:
                                    CNN_train(0,world_size,temp_sweep_config,sweep_mode,sweep_id,run_id_container,total_channel_names,X_train, y_train,\
                                                    X_test, y_test, TrainingDatasets_mean, TrainingDatasets_std,width,height, \
                                                    Evaluation_type,typeName,Temporal_CV_training_begindates[imodel],\
                                                    Temporal_CV_training_enddates[imodel],ifold)
                            ...
                            Daily_Model = load_daily_datesbased_model(evaluation_type=Evaluation_type, typeName=typeName, begindates=Temporal_CV_training_begindates[imodel],
                                                                        enddates=Temporal_CV_training_enddates[imodel], version=version,species=species,
                            ...
                            if not Use_saved_models_to_reproduce_validation_results_Temporal_CV:
                                if world_size > 1:
                                    mp.spawn(CNN_Transformer_train,args=(world_size,temp_sweep_config,sweep_mode,sweep_id,run_id_container,CNN_Embedding_channel_names,Transformer_Embedding_channel_names,
                                                        ...
                                                        Evaluation_type,typeName,Temporal_CV_training_begindates[imodel],Temporal_CV_training_enddates[imodel],ifold),nprocs=world_size)
                                else:
                                    CNN_Transformer_train(0,world_size,temp_sweep_config,sweep_mode,sweep_id,run_id_container,CNN_Embedding_channel_names,Transformer_Embedding_channel_names,
                                                        X_train_CNN, X_test_CNN,X_train_Transformer, X_test_Transformer,
                                                        y_train, y_test,Transformer_trainingdatasets_mean, Transformer_trainingdatasets_std, width,height,
                                                        Evaluation_type,typeName,Temporal_CV_training_begindates[imodel],Temporal_CV_training_enddates[imodel],ifold)
            ...
            save_data_recording(final_data_recording=final_data_recording, obs_data_recording=obs_data_recording, geo_data_recording=geo_data_recording,
                                ...
                                species=species,version=version,begindates=Temporal_CV_training_begindates[0],
                                enddates=Temporal_CV_training_enddates[-1],typeName=typeName,nchannel=len(main_stream_channel_names),
                                evaluation_type=Evaluation_type,project=project,entity=entity,sweep_id=sweep_id,name=name,**args)
            
    

    final_data_recording, obs_data_recording, geo_data_recording, sites_recording, dates_recording, training_final_data_recording, training_obs_data_recording, training_sites_recording, training_dates_recording, sites_lat_array, sites_lon_array = load_data_recording(species=species,version=version,begindates=Temporal_CV_training_begindates[0],
                                                                                                                                                                                                                                         enddates=Temporal_CV_training_enddates[-1],typeName=typeName,nchannel=len(main_stream_channel_names),
    ...
    Daily_statistics_recording, Monthly_statistics_recording, Annual_statistics_recording = calculate_statistics(test_begindates=Temporal_CV_validation_begindates[0],
                                                                                                                test_enddates=Temporal_CV_validation_enddates[-1],final_data_recording=final_data_recording,
    ...
    csvfile_outfile = get_csvfile_outfile(Evaluation_type=Evaluation_type,typeName=typeName,Model_structure_type=Model_structure_type,
                                          main_stream_channel_names=main_stream_channel_names,test_begindate=Temporal_CV_validation_begindates[0],
                                          test_enddate=Temporal_CV_validation_enddates[-1],project=project,entity=entity,sweep_id=sweep_id,
                                          **args,)
    output_csv(outfile=csvfile_outfile,status='w',Area='North America',
                test_begindate=Temporal_CV_validation_begindates[0],test_enddate=Temporal_CV_validation_enddates[-1],
    ...
    if Temporal_CV_regression_plot_switch:
        
        for ifigure in range(len(Temporal_CV_plot_begindates)):
            plot_begin_date = Temporal_CV_plot_begindates[ifigure]
            plot_end_date = Temporal_CV_plot_enddates[ifigure]
    ...
    for idate in range(len(Temporal_CV_validation_begindates)):
        test_begindate =  Temporal_CV_validation_begindates[idate]
        test_enddate = Temporal_CV_validation_enddates[idate]          
